...
export function FinancialTrendChart({ data }: { data: TrendItem[] }) {
  ...
  return <figure className="space-y-4" aria-label="Evolução de receitas e despesas realizadas nos últimos seis meses">
    <div className="flex gap-4 text-xs font-medium text-muted-foreground" aria-hidden="true">
      <span className="flex items-center gap-2"><i className="h-2.5 w-2.5 rounded-full bg-emerald-600" />Receitas</span>
      <span className="flex items-center gap-2"><i className="h-2.5 w-2.5 rounded-full bg-rose-500" />Despesas</span>
    </div>
    <div className="grid h-56 grid-cols-6 items-end gap-2 rounded-xl bg-slate-50 p-3 dark:bg-slate-950/30" aria-hidden="true">
      {data.map((item) => <div key={item.competencia} className="flex h-full min-w-0 flex-col justify-end gap-2">
        <div className="flex flex-1 items-end justify-center gap-1">
          <div className="w-2.5 rounded-t bg-emerald-600 sm:w-4" style={{ height: `${Math.max(2, item.receitas / maximum * 100)}%` }} title={`Receitas: ${brl(item.receitas)}`} />
          <div className="w-2.5 rounded-t bg-rose-500 sm:w-4" style={{ height: `${Math.max(2, item.despesas / maximum * 100)}%` }} title={`Despesas: ${brl(item.despesas)}`} />
        </div>
